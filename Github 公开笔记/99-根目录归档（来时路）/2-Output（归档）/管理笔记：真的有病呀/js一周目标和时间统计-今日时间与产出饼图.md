...
const todayFile = app.vault.getAbstractFileByPath(`${CONFIG.JOURNAL_PATH}/${today}.md`);
let todayTimeEntries = [];

if (todayFile) {
    try {
        const content = await dv.io.load(todayFile.path);
        todayTimeEntries = extractTimeData(content, today);
        
        // 按时间倒序排序（最新的在最前面）
        todayTimeEntries.sort((a, b) => {
        ...
        console.error(`读取今日文件错误: ${todayFile.path}`, e);
    }
}

// 显示今日时间分配和活动数量饼图（并列显示）
dv.span(`
<div style="
    display: grid;
...
">
    ...
    ">
        <h3 style="color: ${CONFIG.THEME.primary}; margin-top: 0; text-align: center;">⏱️ 今日时间分配</h3>
        <div id="today-time-pie" style="height:300px;"></div>
    </div>
    ...
    <div style="
        ...
        box-shadow: 0 2px 6px rgba(0,0,0,0.05);
    ">
        <h3 style="color: ${CONFIG.THEME.primary}; margin-top: 0; text-align: center;">📊 今日活动数量</h3>
        <div id="today-quantity-pie" style="height:300px;"></div>
    </div>
</div>
`);
...
try {
    ...
    todayTimeEntries.forEach(entry => {
        if (!todayTimeData[entry.category]) {
            todayTimeData[entry.category] = 0;
        }
        todayTimeData[entry.category] += entry.minutes;
    });
    
    const todayTimePieData = Object.keys(todayTimeData).map(category => ({
        name: category,
        value: todayTimeData[category]
    }));
    
    // 准备今日活动数量数据
    const today = weeklyData.find(d => d.isToday);
    const todayQuantityPieData = Object.entries(CONFIG.CATEGORIES)
        .filter(([cat]) => today.data[cat] > 0)
        .map(([cat, cfg]) => ({
            name: cat,
            value: today.data[cat],
            unit: cfg.unit
        }));
    
    // 渲染今日时间饼图
    renderTodayTimePieChart(todayTimePieData);
    
    // 渲染今日数量饼图
    renderTodayQuantityPieChart(todayQuantityPieData);
} catch(e) {
...
// 今日时间饼图渲染函数
function renderTodayTimePieChart(data) {
    const chartDom = document.getElementById('today-time-pie');
    if (!chartDom) return;
    ...
    const option = {
        tooltip: {
            trigger: 'item',
            formatter: function(params) {
                return `${params.name}: ${formatTime(params.value)}`;
            }
        },
        series: [
            {
                ...
                data: data.map(item => ({
                    ...
                    value: item.value,
                    itemStyle: {
                        color: CONFIG.TIME_CATEGORIES[item.name]?.color || '#4361ee'
                    }
                })),
                emphasis: {
                    itemStyle: {
...
// 今日活动数量饼图渲染函数
function renderTodayQuantityPieChart(data) {
    const chartDom = document.getElementById('today-quantity-pie');
    if (!chartDom) return;
        ...
        series: [
            {
                ...
                data: data.map(item => ({
                    ...
                    unit: item.unit,
                    itemStyle: {
                        color: CONFIG.CATEGORIES[item.name]?.colors[1] || '#4361ee'
                    }
                })),
                emphasis: {
                    itemStyle: {
...
while (current.isSameOrBefore(yearEnd)) {
    const quarter = getQuarter(current);
    if (!timeGroups.year[quarter]) {
        timeGroups.year[quarter] = {};
        Object.keys(CONFIG.CATEGORIES).forEach(cat => {
            timeGroups.year[quarter][cat] = 0;
        });
    }
    current.add(1, 'month');
}

current = quarterStart.clone();
while (current.isSameOrBefore(quarterEnd)) {
    const month = current.format('YYYY-MM');
    if (!timeGroups.quarter[month]) {
        timeGroups.quarter[month] = {};
        Object.keys(CONFIG.CATEGORIES).forEach(cat => {
            timeGroups.quarter[month][cat] = 0;
        });
    }
    current.add(1, 'month');
}

current = monthStart.clone();
while (current.isSameOrBefore(monthEnd)) {
    const week = `W${current.isoWeek()}`;
    if (!timeGroups.month[week]) {
        timeGroups.month[week] = {};
        Object.keys(CONFIG.CATEGORIES).forEach(cat => {
            timeGroups.month[week][cat] = 0;
        });
    }
    current.add(1, 'week');
}
...
const journalFiles = app.vault.getFiles().filter(file => 
    file.path.startsWith(CONFIG.JOURNAL_PATH) && file.path.endsWith('.md')
);
...
await Promise.all(journalFiles.map(async (file) => {
    ...
    if (fileDate.isBetween(yearStart, yearEnd, null, '[]')) {
        const quarter = getQuarter(fileDate);
        const content = await dv.io.load(file.path);
        
        Object.entries(CONFIG.CATEGORIES).forEach(([cat, cfg]) => {
            const matches = [...content.matchAll(cfg.regex)];
            const count = matches.reduce((sum, match) => {
                return sum + parseFloat(match[1]);
            }, 0);
            
            timeGroups.year[quarter][cat] += count;
        });
    }
    
    if (fileDate.isBetween(quarterStart, quarterEnd, null, '[]')) {
        const month = fileDate.format('YYYY-MM');
        const content = await dv.io.load(file.path);
        ...
        Object.entries(CONFIG.CATEGORIES).forEach(([cat, cfg]) => {
            ...
            timeGroups.quarter[month][cat] += count;
        });
    }
    
    if (fileDate.isBetween(monthStart, monthEnd, null, '[]')) {
        const week = `W${fileDate.isoWeek()}`;
        const content = await dv.io.load(file.path);
        ...
        Object.entries(CONFIG.CATEGORIES).forEach(([cat, cfg]) => {
            ...
            timeGroups.month[week][cat] += count;
        });
    }
}));
...
">
    <div>
        <div style="font-weight: bold; text-align: center; margin-bottom: 10px;">
            ${yearStart.format('YYYY')}年季度统计
        </div>
        <div id="yearly-chart" style="height:300px;"></div>
    </div>
    <div>
        <div style="font-weight: bold; text-align: center; margin-bottom: 10px;">
            ${quarterStart.format('YYYY')}年${getQuarter(quarterStart)}季度月度统计
        </div>
        <div id="quarterly-chart" style="height:300px;"></div>
    </div>
    <div>
        <div style="font-weight: bold; text-align: center; margin-bottom: 10px;">
            ${monthStart.format('YYYY-MM')}月周统计
        </div>
...
function renderYearlyChart() {
    ...
    const myChart = echarts.init(chartDom);
    const quarters = Object.keys(timeGroups.year).sort();
    ...
    const option = {
        ...
        legend: {
            data: Object.keys(CONFIG.CATEGORIES).map(cat => `${CONFIG.CATEGORIES[cat].icon} ${cat}`),
            bottom: 0
        },
        grid: {
            left: '3%',
        ...
        series: Object.keys(CONFIG.CATEGORIES).map(cat => {
            return {
                name: `${CONFIG.CATEGORIES[cat].icon} ${cat}`,
                type: 'bar',
                stack: 'total',
                emphasis: { focus: 'series' },
                data: quarters.map(q => timeGroups.year[q][cat]),
                itemStyle: { color: CONFIG.CATEGORIES[cat].colors[0] }
            }
        })
    };
...
function renderQuarterlyChart() {
    const chartDom = document.getElementById('quarterly-chart');
    if (!chartDom) return;
    
    const myChart = echarts.init(chartDom);
    const months = Object.keys(timeGroups.quarter).sort();
    ...
    const option = {
        ...
        xAxis: {
            type: 'category',
            data: months.map(m => m.substring(5)) // 只显示月份部分
        },
        yAxis: {
            type: 'value',
            name: '活动数量'
        ...
        series: Object.keys(CONFIG.CATEGORIES).map(cat => {
            return {
                ...
                stack: 'total',
                emphasis: { focus: 'series' },
                data: months.map(m => timeGroups.quarter[m][cat]),
                itemStyle: { color: CONFIG.CATEGORIES[cat].colors[1] }
            }
        })
    };
...
function renderMonthlyChart() {
    ...
    const myChart = echarts.init(chartDom);
    const weeks = Object.keys(timeGroups.month).sort();
    ...
    const option = {
        ...
        series: Object.keys(CONFIG.CATEGORIES).map(cat => {
            return {
                ...
                stack: 'total',
                emphasis: { focus: 'series' },
                data: weeks.map(w => timeGroups.month[w][cat]),
                itemStyle: { color: CONFIG.CATEGORIES[cat].colors[2] }
            }
        })
    };
...
<div style="
    ...
    box-shadow: 0 2px 6px rgba(0,0,0,0.05);
">
    <h3 style="color: ${CONFIG.THEME.primary}; margin-top: 0">📅 每日活跃度</h3>
    <div style="display: grid; grid-template-columns: repeat(7, 1fr); gap: 8px;">
    ${weeklyData.map(day => {
        const total = Object.values(day.data).reduce((sum, val) => sum + (val || 0), 0);
        const activityLevel = Math.min(5, Math.floor(total / 5)); // 活跃度等级1-5
        
        return `
    <div style="
        background: ${day.isToday ? CONFIG.THEME.primary + '10' : '#f8f9fa'};
        border: 1px solid ${day.isToday ? CONFIG.THEME.primary + '30' : '#eee'};
        border-radius: 6px;
        padding: 10px;
        text-align: center;
    ">
        <div style="font-weight: bold; color: ${day.isToday ? CONFIG.THEME.primary : '#495057'}; margin-bottom: 6px;">
            ${day.weekday.substring(0,2)}
        </div>
        <div style="font-size: 0.8em; color: #6c757d; margin-bottom: 6px;">
            ${day.date.substring(5)}
        </div>
        <div style="font-size: 1.5em; color: ${CONFIG.THEME.primary};">
            ${'⭐'.repeat(activityLevel) || '—'}
        </div>
...
在配置区的 \`CATEGORIES\` 对象中添加新条目，格式如下：

\`\`\`javascript
"事项名称": { 
    unit: "单位",          // 计量单位（如：公里、个）
    icon: "图标",          // 表情符号图标
    colors: ["#颜色1", "#颜色2", "#颜色3"], // 图表颜色
    regex: /匹配模式/,     // 提取数量的正则
    timeRegex: /匹配模式/, // 提取时间的正则
    target: 目标值,       // 每周目标数量
    timeTarget: 目标时间,  // 每周目标时间（分钟）
    rewards: ["奖励1", "奖励2", "奖励3"] // 神秘奖励
}
\`\`\`
...
"阅读": { 
    unit: "本", 
    icon: "📚", 
    colors: ["#16a085", "#138d75", "#d1f2eb"],
    regex: /阅读.*?(\d+).*?本/gi,
    timeRegex: /阅读.*?【⏳(\d{2}):(\d{2}):(\d{2}) 】/gi,
    target: 3,   // 每周目标阅读3本书
    timeTarget: 300,  // 每周目标阅读300分钟
    rewards: ["📖 私人图书馆体验", "☕ 咖啡馆读书会", "🎧 有声书会员"]
}
\`\`\`

## 🎁 如何自定义随机神秘奖励

1. **分类奖励**: 在每个分类的配置中添加 \`rewards\` 数组
   \`\`\`javascript
   rewards: ["奖励描述1", "奖励描述2", "奖励描述3"]
   \`\`\`

2. **整体奖励**: 在 \`AI_SECRETARY\` 配置中添加 \`specialRewards\` 数组
   \`\`\`javascript
   AI_SECRETARY: {
     // ...其他配置
     specialRewards: ["特别奖励1", "特别奖励2", "特别奖励3"]
   }
   \`\`\`

3. **奖励规则**:
   - 当分类完成率 ≥ 100% 时，随机显示该分类的一个奖励
   - 当完成任意目标时，随机显示一个整体特别奖励
   - 完成所有目标时，显示终极神秘大奖
...
##### 行动记录
  总结：努力成分真的占99%，剩下1%是AI的运气成分，运气真的绝对成功，怎么问AI，用强迫+挖苦它+描述需求清晰=好的答案，最重要，自己会思考，拥有代码调整的技巧，比询问AI等深度思考那么久（搞到凌晨5点）AI结果也是弄不好，这一点我在分组折叠进度条布局的时候用了自己弄好的，深有体会！ ^082b57
- [[2025-06-27]] ⌛️🥊30m做成功✅[[js一周目标和时间统计-今日时间与产出饼图]]问了DeepSeek 300次，连续几天，这应该是DeepSeek调试 charts 的总章了吧！ 【⏳09:00:00 】

| 目标卡片分组：                                                                        | 说明：                                                                                                                  |
| ------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------- |
| **产出价值💰**：<br>产出<br>表达（主动性的）<br>	视频（表达的方式）                                    | 产出≠表达（主动性的）<br>产出包含视频<br>考虑：有想过删掉表达/产出，但为了“适应”多种表达/产出环境，不删还可以有惊喜（模糊了表达/产出方式）<br>更符合我的初心，+1澎拜版<br><br>记住，视频+1，产出和表达+1 |
| **提升和日常**：<br>资讯<br>	使用AI(咨询获取的一种)<br>做餐<br>	用餐（做餐的目的）<br>运动<br>	跑步（运动的方式）<br> | 实践就是，要么是产出/提升，不是方式，就模糊！<br>缺点是优点（想象力），不能视频+1，产出那边就统计+1，除非产出分组就失去了初心澎湃版<br><br>记住，下面只有5个：资讯、使用AI、做餐、用餐、运动、跑步          |
